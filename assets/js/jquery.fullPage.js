        ...
        */
        function setUrlHash(url){
            if(options.recordHistory){
                //Changed it so we can press back button to go to previous page not slide
	                //location.hash = url;
            }else{
...
$(document).ready(function(){
	$('a').click(function(){
    	if($(this).attr('href') && $(this).attr('href').indexOf(".html") != -1 ){
    		window.open($(this).attr('href') , '_self');
    	}
    	else if($(this).hasClass("down-arrow")){
    		$.fn.fullpage.moveSectionDown();
    	}
    	else if($(this).hasClass("up-arrow")){
    		$.fn.fullpage.moveSectionUp();
    	}
    	else if($(this).attr('data-href-slide-number')){
    		if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ){
		    	// Don't need to do any thing. as client says don't scroll on mobile
                $.fn.fullpage.moveTo($(this).attr('data-href-slide-number'));
			}
			else{
	    		$.fn.fullpage.moveTo($(this).attr('data-href-slide-number'));
			}
			...
			return true; // Means the link is not supposed to be handled here, so not interfaring with it
		}
    	return false;
    });
});